...
const Train = ({ data }) => {
  ...
  const [currentPage, setCurrentPage] = useState(
    Number(sessionStorage.getItem("train")) || 1
  );
  ...
  useEffect(() => {
    sessionStorage.setItem("train", currentPage.toString());
  }, [currentPage]);
  // ...

  useEffect(() => {
    const storedPage = Number(sessionStorage.getItem("train")) || 1;
  ...
  const handleClick = (e) => {
    ...
    sessionStorage.setItem("train", pageNumber.toString());
  };
  const pages = [];
  ...
  const handlePrevious = () => {
    ...
    sessionStorage.setItem("train", newPage.toString());
      ...
      <div className="mt-5 mb-24">
        <div className="flex items-center justify-between px-8 mb-5">
          ...
          <div className={style.addHotel}>
            <Link href="/b2bdashboard/manage/train/add">
              <button>
                <span className="text-xl font-bold">+</span> Add Train Package{" "}
              </button>
            </Link>
          </div>
        </div>
...
export async function getStaticProps() {
  try {
    const response = await axios.get("http://localhost:5000/api/v1/train");
